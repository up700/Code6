...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, full_label_ids, span_label_ids, type_label_ids, label_mask, entity_mask, input_ids_ctx):
        ...
        self.input_ids_ctx = input_ids_ctx

def read_examples_from_file(args, data_dir, mode):
    if mode == "train":
        dataset_name = "aug"+args.dataset+args.samples if args.aug else args.dataset+args.samples
        file_path = os.path.join(data_dir+"/"+args.samples, "{}_{}.json".format(dataset_name, mode))
    else:
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for word, span_label, type_label in zip(example.words, example.spans, example.types):
            ...
            if len(word_tokens) > 0:
                ...
                if span_label == tag_to_id["span"]["O"]:
                    entity_mask.extend([0]*len(word_tokens))
                    tokens_ctx.extend(word_tokens)
                else:
                    entity_mask.extend([1]*len(word_tokens))  #######$$$$$$$$
                    tokens_ctx.extend([tokenizer.mask_token]*len(word_tokens))
        ...
        if len(tokens) > max_seq_length - special_tokens_count:
            ...
            entity_mask = entity_mask[: (max_seq_length - special_tokens_count)]
            tokens_ctx = tokens_ctx[: (max_seq_length - special_tokens_count)]
        ...
        if cls_token_at_end:
            ...
            label_mask += [0]
            entity_mask += [0]
            tokens_ctx = [cls_token] + tokens_ctx

        input_ids = tokenizer.convert_tokens_to_ids(tokens)
        input_ids_ctx = tokenizer.convert_tokens_to_ids(tokens_ctx)
        ...
        if pad_on_left:
            ...
            entity_mask = ([0] * padding_length) + entity_mask
            input_ids_ctx = ([pad_token] * padding_length) + input_ids_ctx
        else:
            ...
            input_ids_ctx += [pad_token] * padding_length
        ...
        assert len(input_ids_ctx) == max_seq_length
        ...
        if ex_index < show_exnum:
            ...
            logger.info("tokens_ctx: %s", " ".join([str(x) for x in tokens_ctx]))
            logger.info("input_ids_ctx: %s", " ".join([str(x) for x in input_ids_ctx]))
        ...
        features.append(
            InputFeatures(input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids, \
                ...
                entity_mask=entity_mask, input_ids_ctx=input_ids_ctx)
        )
...
def load_and_cache_examples(args, tokenizer, pad_token_label_id, mode):
    ...
    # tags_to_id_src = tag_to_id(args.data_dir, "conll2003")
    ...
    if mode == "train":
        cached_features_file = os.path.join(
            args.data_dir,
            "{}_{}.pt".format(
                args.dataset+args.samples, mode
            ),
        )
    else:
        cached_features_file = os.path.join(
    ...
    if mode != "train" and os.path.exists(cached_features_file) and not args.overwrite_cache:
    ...
    all_input_ids_ctx = torch.tensor([f.input_ids_ctx for f in features], dtype=torch.long)
    all_ids = torch.tensor([f for f in range(len(features))], dtype=torch.long)

    dataset = TensorDataset(all_input_ids, all_input_mask, all_span_label_ids, all_type_label_ids, all_label_mask, all_entity_mask, all_input_ids_ctx, all_ids)
